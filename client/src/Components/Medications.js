...
const Medications = (props) => {
	...
	const handleAddUpdateMed = async (e, form, cb) => {
		...
		if (existing) {
			url = `${process.env.REACT_APP_BACKEND_API_URL}/api/updatePetMed/${petId}/${form.addMedForm.medId.value}`;
		} else {
			url = `${process.env.REACT_APP_BACKEND_API_URL}/api/addpetmed/${petId}`;
		}
	...
	const handleDelMed = async (e, form, cb) => {
		...
		const url = `${process.env.REACT_APP_BACKEND_API_URL}/api/delPetMed/${petId}/${medId}`;
