...
const VetVists = (props) => {
  ...
  const handleAddUpdateVisit = async (e, form, cb) => {
    ...
    if (existing) {
      url = `${process.env.REACT_APP_BACKEND_API_URL}/api/updatePetVisit/${petId}/${visitId}`;
    } else {
      url = `${process.env.REACT_APP_BACKEND_API_URL}/api/addPetVisit/${petId}`;
    }
    return cb(url, vals);
  };
  ...
  const handleDelVisit = async (e, form, cb) => {
    ...
    let visitId = form.addVisitForm.visitId.value;
    let vals = {};
    let url = `${process.env.REACT_APP_BACKEND_API_URL}/api/delPetVisit/${petId}/${visitId}`;
    return cb(url, vals);
  };
