...
const Reminders = (props) => {
	...
	const handleAddUpdateReminder = async (e, form, cb) => {
		...
		if (existing) {
			url = `${process.env.REACT_APP_BACKEND_API_URL}/api/updatePetReminder/${petId}/${reminderId}`;
		} else {
			url = `${process.env.REACT_APP_BACKEND_API_URL}/api/addPetReminder/${petId}`;
		}
	...
	const handleDelReminder = async (e, form, cb) => {
		...
		let reminderId = form.addReminderForm.reminderId.value;
		let vals = {};
		let url = `${process.env.REACT_APP_BACKEND_API_URL}/api/delPetReminder/${petId}/${reminderId}`;
		return cb(url, vals);
	};
